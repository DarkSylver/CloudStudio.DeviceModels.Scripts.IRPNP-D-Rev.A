function parseUplink(device, payload)
{
    ...
    if (jsonPayload.data !== null) {
        ...
        var producDiaria = device.endpoints.byAddress(2);
        

        // Sentencia para Payload Sensor de produccion Input 1
        if (jsonPayload.inp == "Count Sensor produccion")
        {
            ...
            var ultimo = "";
                for (let i = 0; i < data.length; i++) {
                    ...
                    env.log("Count --> ",item.cnt);
                    produccion.updateFlowSensorValueSummation(item.cnt,item.time);
